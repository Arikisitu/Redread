...
class RootSettingsFragment : BasePreferenceFragment(0) {
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        bindPreferenceSummary(
            ...
            R.string.proxy,
            R.string.prefetch_content,
        )

        bindPreferenceSummary(
            "userdata",
            R.string.create_or_restore_backup,
            R.string.periodic_backups,
        )

        bindPreferenceSummary(
            "downloads",
            R.string.manga_save_location,
            R.string.downloads_wifi_only,
        )

        bindPreferenceSummary(
            "tracker",
            R.string.track_sources,
    ...
    private fun bindPreferenceSummary(key: String, @StringRes vararg items: Int) {
        findPreference<Preference>(key)?.summary =
            items.joinToString { getString(it) }
    }
}
