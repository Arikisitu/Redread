...
class AppSettings @Inject constructor(@ApplicationContext context: Context) {
    ...
    private val mangaListBadgesDefault =
        ArraySet(context.resources.getStringArray(R.array.values_list_badges))
        ...
        get() {
            ...
            return if (raw.isNullOrEmpty()) {
                ...
                raw.mapNotNull { x -> NavItem.entries.find(x) }
                    .ifEmpty { listOf(NavItem.EXPLORE) }
            }
        }
        set(value) {
            prefs.edit {
    ...
    val isMainFabEnabled: Boolean
        get() = prefs.getBoolean(KEY_MAIN_FAB, false)
    ...
    val readerControls: Set<ReaderControl>
        get() = prefs.getStringSet(KEY_READER_CONTROLS, null)
            ?.mapNotNullTo(EnumSet.noneOf(ReaderControl::class.java)) {
    ...
    val trackerDownloadStrategy: TrackerDownloadStrategy
        get() = prefs.getEnumValue(
            KEY_TRACKER_DOWNLOAD,
            TrackerDownloadStrategy.DISABLED,
        )
    ...
    val trackSources: Set<String>
        get() = prefs.getStringSet(KEY_TRACK_SOURCES, null)
            ?: setOf(TRACK_FAVOURITES)

    var appPassword: String?
        get() = prefs.getString(KEY_APP_PASSWORD, null)
        set(value) = prefs.edit {
            if (value != null) {
                putString(KEY_APP_PASSWORD, value)
            } else {
                remove(KEY_APP_PASSWORD)
            }
        }

    var sourceAccessPasswordHash: String?
        get() = prefs.getString(KEY_SOURCE_ACCESS_PASSWORD_HASH, null)
        set(value) = prefs.edit {
            if (value != null) {
                putString(KEY_SOURCE_ACCESS_PASSWORD_HASH, value)
            } else {
                remove(KEY_SOURCE_ACCESS_PASSWORD_HASH)
            }
        }

    var sourceAccessPasswordSalt: String?
        get() = prefs.getString(KEY_SOURCE_ACCESS_PASSWORD_SALT, null)
        set(value) = prefs.edit {
            if (value != null) {
                putString(KEY_SOURCE_ACCESS_PASSWORD_SALT, value)
            } else {
                remove(KEY_SOURCE_ACCESS_PASSWORD_SALT)
            }
        }

    var isSourceAccessUnlocked: Boolean
        get() = prefs.getBoolean(KEY_SOURCE_ACCESS_UNLOCKED, false)
        set(value) = prefs.edit { putBoolean(KEY_SOURCE_ACCESS_UNLOCKED, value) }
    ...
    val searchSuggestionTypes: Set<SearchSuggestionType>
        get() = prefs.getStringSet(KEY_SEARCH_SUGGESTION_TYPES, null)?.let { stringSet ->
            stringSet.mapNotNullTo(
                EnumSet.noneOf(SearchSuggestionType::class.java),
            ) { x ->
    ...
    var isBiometricProtectionEnabled: Boolean
        ...
        set(value) = prefs.edit {
            putBoolean(KEY_PROTECT_APP_BIOMETRIC, value)
        }
    ...
    val isContentPrefetchEnabled: Boolean
        get() {
            ...
            val policy = NetworkPolicy.from(
                prefs.getString(KEY_PREFETCH_CONTENT, null),
                NetworkPolicy.NEVER,
            )
    ...
    var userSpecifiedMangaDirectories: Set<File>
        get() {
            ...
            return set.mapNotNullToSet {
                File(it).takeIfReadable()
            }
        }
        set(value) {
            val set = value.mapToSet {
                it.absolutePath
            }
            prefs.edit {
                putStringSet(KEY_LOCAL_MANGA_DIRS, set)
            }
        }
    ...
    var mangaStorageDir: File?
        get() = prefs.getString(KEY_LOCAL_STORAGE, null)?.let {
            ...
            it.exists() && it in userSpecifiedMangaDirectories
        }
        set(value) = prefs.edit {
            if (value == null) {
    ...
    var allowDownloadOnMeteredNetwork: TriStateOption
        get() = prefs.getEnumValue(
            KEY_DOWNLOADS_METERED_NETWORK,
            TriStateOption.ASK,
        )
        set(value) = prefs.edit {
            putEnumValue(KEY_DOWNLOADS_METERED_NETWORK, value)
        }

    val preferredDownloadFormat: DownloadFormat
        get() = prefs.getEnumValue(
            KEY_DOWNLOADS_FORMAT,
    ...
    var tagsBlacklist: Set<String>
        ...
        set(value) = prefs.edit {
            putString(KEY_TAGS_BLACKLIST, value.joinToString(", "))
        }
    ...
    var readerColorFilter: ReaderColorFilter?
        get() = runCatching {
            ReaderColorFilter(
                brightness = prefs.getFloat(
                    KEY_CF_BRIGHTNESS,
                    ReaderColorFilter.EMPTY.brightness,
                ),
                contrast = prefs.getFloat(
                    KEY_CF_CONTRAST,
                    ReaderColorFilter.EMPTY.contrast,
                ),
                isInverted = prefs.getBoolean(
                    KEY_CF_INVERTED,
                    ReaderColorFilter.EMPTY.isInverted,
                ),
                isGrayscale = prefs.getBoolean(
                    KEY_CF_GRAYSCALE,
                    ReaderColorFilter.EMPTY.isGrayscale,
                ),
                isBookBackground = prefs.getBoolean(
                    KEY_CF_BOOK,
                    ReaderColorFilter.EMPTY.isBookBackground,
                ),
            ).takeUnless {
                it.isEmpty
            }
    ...
    val imagesProxy: Int
        get() {
            ...
            return raw ?: if (prefs.getBoolean(KEY_IMAGES_PROXY_OLD, false)) {
                0
            } else {
                -1
            }
    ...
    val proxyType: Proxy.Type
        get() {
            val raw = prefs.getString(KEY_PROXY_TYPE, null)
                ?: return Proxy.Type.DIRECT

            return enumValues<Proxy.Type>().find {
                it.name == raw
            } ?: Proxy.Type.DIRECT
        }
    ...
    val defaultWebtoonZoomOut: Float
        get() = prefs.getInt(KEY_WEBTOON_ZOOM_OUT, 0)
            .coerceIn(0, 50) / 100f
    ...
    var isReaderAutoscrollFabVisible: Boolean
        ...
        set(value) = prefs.edit {
            putBoolean(KEY_READER_AUTOSCROLL_FAB, value)
        }
    ...
    var discordToken: String?
        ...
        set(value) = prefs.edit {
            putString(KEY_DISCORD_TOKEN, value?.nullIfEmpty())
        }
    ...
    val periodicalBackupFrequency: Float
        get() = prefs.getString(
            KEY_BACKUP_PERIODICAL_FREQUENCY,
            null,
        )?.toFloatOrNull() ?: 7f
    ...
    var gitHubMirror: GitHubMirror
        ...
        set(value) = prefs.edit {
            putEnumValue(KEY_GITHUB_MIRROR, value)
        }
    ...
    var downloadOffPeakStart: String
        get() = prefs.getString(KEY_DOWNLOAD_OFF_PEAK_START, "00:00")
            ?: "00:00"
        set(value) = prefs.edit {
            putString(KEY_DOWNLOAD_OFF_PEAK_START, value)
        }

    var downloadOffPeakEnd: String
        get() = prefs.getString(KEY_DOWNLOAD_OFF_PEAK_END, "06:00")
            ?: "06:00"
        set(value) = prefs.edit {
            putString(KEY_DOWNLOAD_OFF_PEAK_END, value)
        }

    var isDownloadOffPeakEnabled: Boolean
        get() = prefs.getBoolean(KEY_DOWNLOAD_OFF_PEAK_ENABLED, false)
        set(value) = prefs.edit {
            putBoolean(KEY_DOWNLOAD_OFF_PEAK_ENABLED, value)
        }

    var downloadStorageQuota: Long
        get() = prefs.getString(
            KEY_DOWNLOAD_STORAGE_QUOTA,
            null,
        )?.toLongOrNull() ?: 0L
        set(value) = prefs.edit {
            putString(
    ...
    fun isPagesCropEnabled(mode: ReaderMode): Boolean {
        val rawValue = prefs.getStringSet(
            KEY_READER_CROP,
            emptySet(),
        )
        ...
        val needle = if (mode == ReaderMode.WEBTOON) {
            READER_CROP_WEBTOON
        } else {
            READER_CROP_PAGED
        }
    ...
    fun closeTip(tip: String) {
        val closedTips = prefs.getStringSet(
            KEY_TIPS_CLOSED,
            emptySet(),
        ).orEmpty()
    ...
    fun getMangaListBadges(): Int {
        val raw = prefs.getStringSet(
            KEY_MANGA_LIST_BADGES,
            mangaListBadgesDefault,
        ).orEmpty()
    ...
    companion object {
        ...
        const val KEY_READER_DOUBLE_PAGES_SENSITIVITY =
            "reader_double_pages_sensitivity_2"
        ...
        const val KEY_SOURCE_ACCESS = "source_access"
        const val KEY_SOURCE_ACCESS_PASSWORD_HASH =
            "source_access_password_hash"
        const val KEY_SOURCE_ACCESS_PASSWORD_SALT =
            "source_access_password_salt"
        const val KEY_SOURCE_ACCESS_UNLOCKED =
            "source_access_unlocked"
        ...
        const val KEY_SUGGESTIONS_DISABLED_SOURCES =
            "suggestions_disabled_sources"
        const val KEY_SUGGESTIONS_NOTIFICATIONS =
            "suggestions_notifications"
        const val KEY_SHIKIMORI = "shikimori"
        const val KEY_ANILIST = "anilist"
        const val KEY_MAL = "mal"
        const val KEY_KITSU = "kitsu"
        const val KEY_DOWNLOADS_METERED_NETWORK =
            "downloads_metered_network"
        const val KEY_DOWNLOADS_FORMAT = "downloads_format"
        const val KEY_ALL_FAVOURITES_VISIBLE =
            "all_favourites_visible"
        ...
        const val KEY_READER_BAR_TRANSPARENT =
            "reader_bar_transparent"
        const val KEY_READER_CHAPTER_TOAST =
            "reader_chapter_toast"
        ...
        const val KEY_DISCORD_RPC_SKIP_NSFW =
            "discord_rpc_skip_nsfw"
        const val KEY_DISCORD_TOKEN = "discord_token"
        const val KEY_CRASH_ANALYTICS_ENABLED =
            "crash_analytics_enabled"
        const val KEY_GITHUB_MIRROR = "github_mirror"

        const val KEY_DOWNLOAD_OFF_PEAK_START =
            "download_off_peak_start"
        const val KEY_DOWNLOAD_OFF_PEAK_END =
            "download_off_peak_end"
        const val KEY_DOWNLOAD_OFF_PEAK_ENABLED =
            "download_off_peak_enabled"
        const val KEY_DOWNLOAD_STORAGE_QUOTA =
            "download_storage_quota"
        const val KEY_AUTO_DOWNLOAD_NEXT =
            "auto_download_next"
        const val KEY_DOWNLOAD_BANDWIDTH_LIMIT =
            "download_bandwidth_limit"
        const val KEY_DOWNLOAD_ONLY_ON_CHARGING =
            "download_only_on_charging"
